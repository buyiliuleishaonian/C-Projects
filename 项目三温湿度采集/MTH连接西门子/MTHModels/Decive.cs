...
namespace MTHModels
{
    ...
    public class Decive
    {
        ...
        {
            string alarm= var.VarName.Substring(0,4);
            //在判断是上升沿报警还是下降演报警之前，需要判断温湿度报警是否启用
            if (this.GroupList[0].VarList.Where(c=>c.VarName==alarm+"报警启用").ToList().Count>0)
            {
                //该模块的报警是否启用
                if ((int)this.CurrentVariable[alarm+"报警启用"]==1)
                {
                    //判断是启动上升沿还是下降报警
                    if (var.PosAlarm)
                    {
